...
public class CheckoutController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        if (TempData["AddressError"] is string addrErr) TempData["AddressError"] = addrErr;
        if (TempData["PhoneError"] is string phoneErr) TempData["PhoneError"] = phoneErr;
    ...
    public async Task<IActionResult> PlaceCod(string? couponCode, string? shippingAddress, string? phoneNumber)
    {
        ...
        var phoneValid = !string.IsNullOrWhiteSpace(phoneNumber) && System.Text.RegularExpressions.Regex.IsMatch(phoneNumber.Trim(), @"^[0-9]{9,11}$");
        if (string.IsNullOrWhiteSpace(shippingAddress) || !phoneValid)
        {
            TempData["ShippingAddress"] = shippingAddress;
            TempData["PhoneNumber"] = phoneNumber;
            ...
            if (string.IsNullOrWhiteSpace(phoneNumber))
                ...
                TempData["PhoneError"] = "Số điện thoại chỉ gồm chữ số, độ dài 9–11 ký tự.";
            return RedirectToAction(nameof(Index));
        }

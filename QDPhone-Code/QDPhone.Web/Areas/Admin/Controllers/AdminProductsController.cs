...
public class AdminProductsController : Controller
{
    ...
    public async Task<IActionResult> Create(ProductCreateViewModel model, IFormFile? primaryImageFile)
    {
        ...
        {
            // Gán ảnh luôn cho variant vừa tạo
    ...
    public async Task<IActionResult> Edit(int id, ProductCreateViewModel model, IFormFile? primaryImageFile)
    {
        ...
        if (!string.IsNullOrWhiteSpace(model.PrimaryImageUrl))
        {
            // Tìm ảnh primary của variant này
            var variantPrimary = product.Images.FirstOrDefault(i => i.VariantId == variant.Id && i.IsPrimary);
            if (variantPrimary == null)
            {
                _db.ProductImages.Add(new ProductImage
                {
                    ProductId = product.Id,
            ...
            {
                variantPrimary.ImageUrl = model.PrimaryImageUrl.Trim();
            }
        }
    ...
    /// <summary>
    /// Quản lý ảnh: nếu truyền variantId thì hiển thị ảnh của variant đó,
    /// nếu không thì hiển thị ảnh chung (VariantId IS NULL).
    /// </summary>
    [HttpGet("{productId:int}/images")]
    public async Task<IActionResult> Images(int productId, int? variantId)
    {
        var product = await _db.Products
            .Include(x => x.Variants)
            .Include(x => x.Images)
    ...
    public async Task<IActionResult> AddImage(int productId, string? imageUrl, IFormFile? imageFile, int? variantId)
    {
        if (string.IsNullOrWhiteSpace(imageUrl) && imageFile == null)
            return RedirectToAction(nameof(Images), new { productId, variantId });
        ...
        // SortOrder tính riêng theo (ProductId, VariantId)
        var existingForScope = product.Images
            .Where(x => x.VariantId == variantId)
            .ToList();
        var nextSort = existingForScope.Any() ? existingForScope.Max(x => x.SortOrder) + 1 : 0;
        ...
        _db.ProductImages.Add(new ProductImage
        {
            ...
            IsPrimary = !existingForScope.Any(),
            SortOrder = nextSort
        });
        await _db.SaveChangesAsync();
        return RedirectToAction(nameof(Images), new { productId, variantId });
    ...
    public async Task<IActionResult> SetPrimaryImage(int productId, int imageId, int? variantId)
    {
        var images = await _db.ProductImages
            .Where(x => x.ProductId == productId && x.VariantId == variantId)
            .ToListAsync();
        if (!images.Any()) return RedirectToAction(nameof(Images), new { productId, variantId });
    ...
    public async Task<IActionResult> UpdateSortOrder(int productId, int imageId, int sortOrder, int? variantId)
    {
        var image = await _db.ProductImages.FirstOrDefaultAsync(x => x.Id == imageId && x.ProductId == productId);
        if (image == null) return RedirectToAction(nameof(Images), new { productId, variantId });
    ...
    [HttpPost]
    [Route("{productId:int}/images/move")]
    public async Task<IActionResult> MoveImage(int productId, int imageId, int? variantId, string? targetScope, int? targetVariantId)
    {
        var image = await _db.ProductImages.FirstOrDefaultAsync(x => x.Id == imageId && x.ProductId == productId);
        if (image == null) return RedirectToAction(nameof(Images), new { productId, variantId });

        // Xác định scope đích
        int? newVariantId = targetScope == "shared" ? null : targetVariantId;

        // Nếu không thay đổi thì bỏ qua
        if (image.VariantId == newVariantId)
            return RedirectToAction(nameof(Images), new { productId, variantId });

        // Tính SortOrder mới trong scope đích
        var imagesInTarget = await _db.ProductImages
            .Where(x => x.ProductId == productId && x.VariantId == newVariantId)
            .ToListAsync();
        var nextSort = imagesInTarget.Any() ? imagesInTarget.Max(x => x.SortOrder) + 1 : 0;
        var noPrimaryInTarget = !imagesInTarget.Any(x => x.IsPrimary);

        // Nếu ảnh này đang là primary trong scope cũ, cần đặt primary cho ảnh còn lại
        if (image.IsPrimary)
        {
            var remaining = await _db.ProductImages
                .Where(x => x.ProductId == productId && x.VariantId == image.VariantId && x.Id != imageId)
        ...
        image.SortOrder = nextSort;
        image.IsPrimary = noPrimaryInTarget; // Tự động làm primary nếu scope đích chưa có
    ...
    public async Task<IActionResult> DeleteImage(int productId, int imageId, int? variantId)
    {
        var image = await _db.ProductImages.FirstOrDefaultAsync(x => x.Id == imageId && x.ProductId == productId);
        if (image == null) return RedirectToAction(nameof(Images), new { productId, variantId });

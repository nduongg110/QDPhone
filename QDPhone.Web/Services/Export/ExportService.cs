...
public class ExportService : IExportService
{
    public byte[] ExportOrdersToExcel(IEnumerable<OrderExportRowViewModel> rows)
    {
        ...
        foreach (var x in rows)
        {
            ...
            ws.Cell(row, 2).Value = x.UserId;
            ws.Cell(row, 3).Value = x.UserName;
            ws.Cell(row, 4).Value = x.ProductName;
            ws.Cell(row, 5).Value = x.Status;
            ws.Cell(row, 6).Value = x.TotalAmount;
            row++;
        }
